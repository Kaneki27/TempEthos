...
import { ArrowLeft, Edit, ClipboardCheck, Loader2 } from 'lucide-react';
...
export default function EditPatientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    if (match) {
      setPatient(match);
      setName(match.name);
      setNationalId(match.nationalId);
      setGender(match.gender);
      setDob(match.dateOfBirth);
      setBloodType(match.bloodType);
      setHeight(String(match.height));
      setWeight(String(match.weight));
      setAddress(match.address);
      setContactNumber(match.contactNumber);
      
      setEmergencyName(match.emergencyContact.name);
      setEmergencyRelation(match.emergencyContact.relation);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !nationalId || !dob || !contactNumber) return;
    ...
    try {
      ...
      await updatePatient('patients', id, patientPayload);
      setSuccess(true);
      setLoading(false);
      setTimeout(() => {
      ...
      <div className="max-w-3xl mx-auto space-y-6 animate-fade-in-up">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4 text-left">
          <div className="flex items-center gap-3">
            <Link href={`/patients/${id}`} className="p-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition">
              <ArrowLeft className="h-4.5 w-4.5 text-slate-500" />
            </Link>
            <div>
              <h2 className="text-xl font-bold text-slate-800">Edit Patient Profile: {patient.name}</h2>
              <p className="text-xs text-slate-500 mt-0.5">Modify clinical benchmarks and contact profiles.</p>
            </div>
          </div>
        </div>
        ...
        {success ? (
          <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm text-center py-12">
            ...
            <h3 className="mt-4 text-lg font-bold text-slate-800">Changes Saved!</h3>
          ...
          <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6 text-left">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-500 block mb-1.5">National ID No *</label>
                  <input
                    type="text"
                    required
                    value={nationalId}
